...
void lcd_cus_ctrl_dump_info(struct lcd_config_s *pconf)
{
	...
	printf("\ncus_ctrl info:\n"
		...
		pconf->cus_ctrl.ctrl_en,
		pconf->cus_ctrl.ctrl_cnt,
		pconf->cus_ctrl.timing_ctrl_valid,
	...
	for (i = 0; i < pconf->cus_ctrl.ctrl_cnt; i++) {
...
static int lcd_cus_ctrl_attr_parse_ufr_ukey(struct lcd_config_s *pconf,
		struct lcd_cus_ctrl_attr_config_s *attr_conf, unsigned char *p)
{
	...
	memcpy(ptiming, &pconf->lcd_timing.dft_timing, sizeof(struct lcd_detail_timing_s));
...
static int lcd_cus_ctrl_attr_parse_dfr_ukey(struct lcd_config_s *pconf,
		struct lcd_cus_ctrl_attr_config_s *attr_conf, unsigned char *p)
{
	...
	for (i = 0; i < dfr_attr->fr_cnt; i++) {
		...
		if (dfr_attr->fr[i].timing_index == 0) {
			...
			if (n >= dfr_attr->tmg_group_cnt) {
				LCDERR("%s: timing_index %d err, tmg_group_cnt %d\n",
					__func__, dfr_attr->fr[i].timing_index,
	...
	pconf->cus_ctrl.timing_cnt += dfr_attr->fr_cnt;
	attr_conf->attr.dfr_attr = dfr_attr;
	return 0;
}

static int lcd_cus_ctrl_attr_parse_extend_tmg_ukey(struct lcd_config_s *pconf,
		struct lcd_cus_ctrl_attr_config_s *attr_conf, unsigned char *p)
{
	...
	pconf->cus_ctrl.timing_cnt += extend_tmg_attr->group_cnt;
	attr_conf->attr.extend_tmg_attr = extend_tmg_attr;
	return 0;
}

static int lcd_cus_ctrl_attr_parse_clk_adv_ukey(struct lcd_config_s *pconf,
		struct lcd_cus_ctrl_attr_config_s *attr_conf, unsigned char *p)
{
...
static int lcd_cus_ctrl_attr_parse_tcon_sw_pol_ukey(struct lcd_config_s *pconf,
		struct lcd_cus_ctrl_attr_config_s *attr_conf, unsigned char *p)
{
	return 0;
}

static int lcd_cus_ctrl_attr_parse_tcon_sw_pdf_ukey(struct lcd_config_s *pconf,
		struct lcd_cus_ctrl_attr_config_s *attr_conf, unsigned char *p)
{
	return 0;
}
		...
		unsigned int max_size)
{
	...
	pconf->cus_ctrl.ctrl_en = ctrl_en;
	...
	pconf->cus_ctrl.attr_config = attr_conf;
	if (lcd_debug_print_flag) {
		LCDPR("%s: ctrl_en=0x%x, ctrl_cnt=%d\n",
			__func__, ctrl_en, ctrl_cnt);
	}
	...
	for (i = 0; i < LCD_CUS_CTRL_ATTR_CNT_MAX; i++) {
		...
		if (offset >= max_size) {
			if (lcd_debug_print_flag) {
				LCDPR("%s: offset %d reach max_size %d, exit\n",
					__func__, offset, max_size);
			}
			break;
		}
		p = buf + offset;
		...
		if (ctrl_en & (1 << i)) { //valid
			...
			case LCD_CUS_CTRL_TYPE_UFR:
				if (attr_conf[n].attr_flag > 0)
					timing_ctrl_valid = 1;
				ret = lcd_cus_ctrl_attr_parse_ufr_ukey(pconf,
					&attr_conf[n], (p + 4));
				break;
			case LCD_CUS_CTRL_TYPE_DFR:
				if (attr_conf[n].attr_flag > 0)
					timing_ctrl_valid = 1;
				ret = lcd_cus_ctrl_attr_parse_dfr_ukey(pconf,
					&attr_conf[n], (p + 4));
				break;
			case LCD_CUS_CTRL_TYPE_EXTEND_TMG:
				if (attr_conf[n].attr_flag > 0)
					timing_ctrl_valid = 1;
				ret = lcd_cus_ctrl_attr_parse_extend_tmg_ukey(pconf,
					&attr_conf[n], (p + 4));
				break;
			case LCD_CUS_CTRL_TYPE_CLK_ADV:
				ret = lcd_cus_ctrl_attr_parse_clk_adv_ukey(pconf,
					&attr_conf[n], (p + 4));
				break;
			case LCD_CUS_CTRL_TYPE_TCON_SW_POL:
				LCDERR("todo\n");
				ret = lcd_cus_ctrl_attr_parse_tcon_sw_pol_ukey(pconf,
					&attr_conf[n], (p + 4));
				break;
			case LCD_CUS_CTRL_TYPE_TCON_SW_PDF:
				LCDERR("todo\n");
				ret = lcd_cus_ctrl_attr_parse_tcon_sw_pdf_ukey(pconf,
					&attr_conf[n], (p + 4));
				break;
			default:
				LCDERR("%s: invalid attr_type: 0x%x\n",
					__func__, attr_conf[n].attr_type);
	...
	if (timing_ctrl_valid)
		pconf->cus_ctrl.timing_ctrl_valid = timing_ctrl_valid;
	return 0;

lcd_cus_ctrl_load_from_unifykey_err:
	lcd_cus_ctrl_config_remove(pconf);
	return -1;
}
...
void lcd_cus_ctrl_config_remove(struct lcd_config_s *pconf)
{
	...
	size = pconf->cus_ctrl.ctrl_cnt * sizeof(struct lcd_cus_ctrl_attr_config_s);
	memset(pconf->cus_ctrl.attr_config, 0, size);
	free(pconf->cus_ctrl.attr_config);
	pconf->cus_ctrl.attr_config = NULL;
}

static int lcd_cus_ctrl_config_update_clk_adv(struct lcd_config_s *pconf,
		struct lcd_cus_ctrl_attr_config_s *attr_conf)
{
	if (attr_conf->attr_flag & (1 << 0)) {
		pconf->lcd_timing.ss_freq = attr_conf->attr.clk_adv_attr->ss_freq;
		pconf->lcd_timing.ss_mode = attr_conf->attr.clk_adv_attr->ss_mode;
	}
...
int lcd_cus_ctrl_config_update(struct lcd_config_s *pconf, void *param, unsigned int mask_sel)
{
	...
	for (i = 0; i < pconf->cus_ctrl.ctrl_cnt; i++) {
		...
		case LCD_CUS_CTRL_TYPE_CLK_ADV:
			...
			if (!attr_conf->attr.clk_adv_attr)
				break;
			ret = lcd_cus_ctrl_config_update_clk_adv(pconf, attr_conf);
			return ret;
		case LCD_CUS_CTRL_TYPE_TCON_SW_POL:
...
{
	if (pconf->cus_ctrl.timing_ctrl_valid)
		return 1;
...
int lcd_cus_ctrl_timing_is_activated(struct lcd_config_s *pconf)
{
	...
	switch (pconf->cus_ctrl.active_timing_type) {
	case LCD_CUS_CTRL_TYPE_UFR:
	case LCD_CUS_CTRL_TYPE_DFR:
	case LCD_CUS_CTRL_TYPE_EXTEND_TMG:
		if (pconf->cus_ctrl.timing_switch_flag > 0)
			ret = 1;
		break;
	default:
		break;
...
struct lcd_detail_timing_s **lcd_cus_ctrl_timing_match_get(struct lcd_config_s *pconf)
{
	...
	cnt = pconf->cus_ctrl.timing_cnt * sizeof(struct lcd_detail_timing_s *);
	...
	for (i = 0; i < pconf->cus_ctrl.ctrl_cnt; i++) {
		cus_ctrl_attr = &pconf->cus_ctrl.attr_config[i].attr;
		switch (pconf->cus_ctrl.attr_config[i].attr_type) {
		case LCD_CUS_CTRL_TYPE_UFR:
		...
		case LCD_CUS_CTRL_TYPE_EXTEND_TMG:
			...
			for (j = 0; j < cus_ctrl_attr->extend_tmg_attr->group_cnt; j++) {
				...
				if (lcd_debug_print_flag) {
					LCDPR("%s: attr[%d] extend_tmg[%d]: %dx%dp%dhz\n",
						__func__, i, j,
						timing_match[n]->h_active,
						timing_match[n]->v_active,
						timing_match[n]->frame_rate);
